...
class _HomePageState extends State<HomePage> {
        ...
        builder: (context, trackersSnapshot) {
          ...
          return StreamBuilder<List<TrackerHistoryModel>>(
            ...
            builder: (context, historySnapshot) {
              ...
              bool hasTrackerSlipUpOnDay(TrackerModel tracker, DateTime dayDate) {
              ...
              bool isTrackerCompletedOnDay(TrackerModel tracker, DateTime dayDate) {
                ...
                if (tracker.type == 'maintain') {
                  final hasManualCompletion = history.any((h) =>
                      h.trackerId == tracker.id &&
                      h.type == 'completion' &&
                      h.date.year == dayDate.year &&
                      h.date.month == dayDate.month &&
              ...
              final activeOnSelected = trackers.where((tracker) {
                final start = DateTime(tracker.originalStartDate.year, tracker.originalStartDate.month, tracker.originalStartDate.day);
                final day = DateTime(_selectedDay.year, _selectedDay.month, _selectedDay.day);
                if (day.isBefore(start)) return false;
                if (tracker.endDate != null) {
                  final end = DateTime(tracker.endDate!.year, tracker.endDate!.month, tracker.endDate!.day);
          ...
          children: [
            ...
            GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                bool hasTrackerSlipUpOnDay(TrackerModel tracker, DateTime date) {
                  if (tracker.type != 'quit') return false;
                  return history.any((h) =>
                      h.trackerId == tracker.id &&
                      h.type == 'slip_up' &&
                ...
                bool isTrackerCompletedOnDay(TrackerModel tracker, DateTime date) {
                ...
                final completedForDay = trackers.where((t) => isTrackerCompletedOnDay(t, dayDate)).toList();

                final allIndicators = [
                  ...completedForDay.map((t) {
                    return t.type == 'quit'
                        ? const Color(0xFFEF5350) // Red dot matching quit cards
                        : const Color(0xFF26A69A); // Teal dot matching maintain cards
                  }),
                ];
  ...
  Widget _buildDetailPanel({
    ...
    required List<TrackerModel> active,
  }) {
    final formattedDate =
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (active.isEmpty)
                      const Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'No habit trackers were active on this date.',
                                style: TextStyle(color: Colors.grey, fontSize: 14),
                              ),
                            ),
                          ],
                      ...
                      ...active.map((t) {
                        final isDone = completed.any((c) => c.id == t.id);
                        return _buildDetailItem(t, isDone);
                      }),
                  ],
                ),
              ),
  ...
  Widget _buildDetailItem(TrackerModel tracker, bool isCompleted, {bool isSlip = false}) {
